...
// iterate through command functions and assigns the apropriate function to the given command
...
// writes a response consisting of three-digit code and the given text to the fd.
...
// interprets the user command; try to find the username in the database, forces the next command to be PASS
...
// checks whether the combination of the username and password is correct
...
void dele(char **params, short *abor, int fd, struct state *cstate, struct config *configuration) {
	if(!cstate->logged){
		respond(fd, 5, 3, 0, "Not logged in.");
		return;
	}
...
void port(char **params, short *abor, int fd, struct state *cstate, struct config *configuration) {
	...
	while((comma = strchr(params[0], ',')) != NULL){
		// read 4 delimited strings; port now starts at port_st; size equals the length of the IP adrress
		if (++pts == 4){
			size = comma - params[0];
			port_st = comma + 1;
			break;
...
// sends the listing of the current directory through data connection - client part
...
void d_list(char **params, short *abor, int fd, struct state *cstate, struct config *configuration) {
	...
	// loads the listing of the directory determined by path to string pointed to by buf
	listDir(path, buf);
	// sends the list
...
void pasv(char **params, short *abor, int fd, struct state *cstate, struct config *configuration) {
	...
	// reads the port and IP adress and port the data transfer process stated to listen on
...
void retr(char **params, short *abor, int fd, struct state *cstate, struct config *configuration) {
	...
	// loads the full path to the given directory into the filepath variable
...
void d_retr(char **params, short *abor, int fd, struct state *cstate, struct config *configuration) {
	...
	// read blocks of data from the file and sends it through the data connection
	while((r = read(file, buf, BUFSIZE)) > 0){
		usleep(100000);
		// ASCII type transfer
		if(cstate->transfer_type == ASCII){
			char tmp[2 * BUFSIZE];
			// converts \n to \r\n sequence, return how much is the result longer
