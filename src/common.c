...
// read configuaration from the conf.h and process parametres, fills the struct configuration
...
char *changeDir(char *path, char *dir) {
	...
	if (strcmp(dir, "..") == 0){
		...
		if ((strcmp(path, "/") == 0) || (last_slash == NULL)) { // root dir or invalid path
			return (NULL);
		}
...
// loads the file descriptor of the created socket in accepted
short spawnConnection(struct state *cstate, int *accepted){
	*accepted = 0;
	...
	if(!cstate->data_sock){
		...
		if ((connect(*accepted, (struct sockaddr *) in, sizeof(struct sockaddr_in))) == -1) {
...
int listDir(char *dir, char *result){
	...
	while (entry = readdir(d)){
		...
		if((finfo->st_mode & S_IFMT) == S_IFDIR)
			mode[0] = 'd';
		loadPerms((finfo->st_mode & S_IRWXU) >> 6, mode + 1, mode + 2, mode + 3);
		loadPerms((finfo->st_mode & S_IRWXG) >> 3, mode + 4, mode + 5, mode + 6);
		loadPerms((finfo->st_mode & S_IRWXO), mode + 7, mode + 8, mode + 9);
		mode[10] = 0;
		struct tm *timeinfo;
	    ...
	    tm[12] = 0;
		sprintf(line, "%s %d %s %s %d %s %s\n", mode, 1, "FTP", "FTP", (int) finfo->st_size, tm, entry->d_name);
		strcpy(result, line);
		result += strlen(line);
	}
	printf("%s\n", result);
